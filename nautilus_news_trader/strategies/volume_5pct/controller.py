#!/usr/bin/env python3
"""
Volume 5% Strategy Controller.

Spawns NewsVolumeStrategy with 5% of 3-second volume.
Fixed 7-minute exit.
"""
...
class Volume5PctControllerConfig(BaseNewsControllerConfig, frozen=True):
    """Configuration for Volume 5% Controller."""
...
class Volume5PctController(BaseNewsController):
    """Controller that spawns NewsVolumeStrategy at 5% volume."""

    def __init__(self, trader: Trader, config: Optional[Volume5PctControllerConfig] = None):
        if config is None:
            config = Volume5PctControllerConfig()
    ...
    def _calculate_position_size(self, volume_data: dict, trace_id: str) -> float:
        ...
        self.log.info(f"[{trace_id}] Position size: ${position_size:.2f} (5% of ${usd_volume:.2f})")
    ...
    def _spawn_strategy(self, symbol: str, position_size: float, volume_data: dict,
        ...
        try:
            ...
            # Create strategy
            strategy_id = f"vol5_{symbol}_{int(pub_time.timestamp())}"
            ...
            self.log.info(f"[{trace_id}] Spawned vol5 strategy: ${position_size:.2f}")
