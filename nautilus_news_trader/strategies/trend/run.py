#!/usr/bin/env python3
"""
Trend Strategy Runner.

Independent process for NewsTrendStrategy with trend-based entry/exit.
"""
...
def main():
    """Main entry point."""
    logger.info("=" * 60)
    logger.info("TREND STRATEGY (EMA-based entry/exit)")
    logger.info("=" * 60)
    ...
    # Trend thresholds
    trend_entry = float(os.getenv("TREND_ENTRY_THRESHOLD", 95.0))
    trend_exit = float(os.getenv("TREND_EXIT_THRESHOLD", 64.0))

    logger.info(f"Trend entry threshold: >= {trend_entry}")
    logger.info(f"Trend exit threshold: < {trend_exit}")

    # Controller config
    controller_config = ImportableControllerConfig(
        controller_path="strategies.trend.controller:TrendController",
        config_path="strategies.trend.controller:TrendControllerConfig",
        config={
            "project_id": project_id,
            "subscription_id": "benzinga-news-trend",  # Dedicated subscription
            ...
            "trend_entry_threshold": trend_entry,
            "trend_exit_threshold": trend_exit,
            "historical_bars": int(os.getenv("HISTORICAL_BARS", 300)),
    ...
    node_config = TradingNodeConfig(
        trader_id=TraderId("TREND-TRADER"),
        ...
        logging=LoggingConfig(
            ...
            log_file_name=f"trend_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    logger.info("Trend Strategy OPERATIONAL")
